...
'''
from https://github.com/tomrittervg/decrypt-windows-ec2-passwd
'''
class decryptPasswordWindows:

    def __init__(self, profile, password):
        keyfile = os.path.expanduser('~') + '/.ssh/' + profile + '.pem'
        self.__password = password
        try:
            keyFile = open(keyfile)
        except:
            print("Could not find file", keyfile)
            sys.exit(-1)
        #Import it
        try:
    ...
    def decrypt_password(self):
        return  self.__decryptPassword(self.key, self.__password)

    def __pkcs1_unpad(self, text):
    ...
    def __decryptPassword(self, rsaKey, password):
        ...
        decryptedData = self.__long_to_bytes(plaintext)
        #Now Unpad it
        unpaddedData = self.__pkcs1_unpad(decryptedData)
        #Done
